...
{
  //float mappedX = map(xx, 1728, 0, -39 + (random(3)-1), 33 + (random(3)-1));

  float mappedX = map(xx, 1728, 0, -39, 34);

  //float mappedZ = map(zz, 0, 960, 20 + (random(3)-1), -27 + (random(3)-1));
  float mappedZ = map(zz, 0, 960, 20, -27);

  float mappedArea = map(area, 3000, 30000, 5, 8);//check med blobs
  ...
  println("FPS = " + int(frameRate) + "  ID: " + id + "  X = " + xx + " (" + mappedX + ")" +  "  Y (Mapped) = " + zz + " (" + mappedZ + ")" + "  Area = " + mappedArea);
}
